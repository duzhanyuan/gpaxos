...
type MembershipChangedCallback func(idx int32, list gpaxos.NodeInfoList)
...
func (self *SystemStateMachine) GetMembership(nodes gpaxos.NodeInfoList, version *uint64) {
...
func (self *SystemStateMachine) Membership_OPValue(nodes gpaxos.NodeInfoList, version uint64, value []byte) error {
...
func (self *SystemStateMachine) AddNodeIDList(nodes gpaxos.NodeInfoList) {
...
func (self *SystemStateMachine) RefleshNodeID() {
  self.nodeIdMap = make(map[uint64]bool)
  nodes := make([]gpaxos.NodeInfo, 0)

  for _, node := range self.systemVariables.GetMemberShip() {
    nodeInfo := gpaxos.NodeInfo{
      NodeId: node.GetNodeid(),
    }
