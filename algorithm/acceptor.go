...
func (self *Acceptor) OnPrepare(msg common.PaxosMsg) error {
    ...
    if ballot.BE(self.acceptorState.GetPromiseNum()) {
        ...
        if self.acceptorState.GetAcceptedNum().proposalId > 0 {
            reply.Value = make([]byte, len(self.acceptorState.AcceptValue))
            copy(reply.Value, self.acceptorState.AcceptValue)
        }
...
func (self *Acceptor) OnAccept(msg common.PaxosMsg) error {
    ...
    if ballot.BE(self.acceptorState.GetPromiseNum()) {
        ...
        self.acceptorState.AcceptValue = make([]byte, len(msg.Value))
        copy(self.acceptorState.AcceptValue, msg.Value)
